...
if [ "{{www_data_path}}" != "{{lm_prefix}}/var/www-data" ]; then
  ...
  if [ ! -d "{{www_data_path}}" ] && [ -d "{{ lm_prefix }}/var/www-data" ]; then
    ...
    mv "{{lm_prefix}}/var/www-data" "{{www_data_path}}" 2>/dev/null || mv "{{lm_prefix}}/var/www-data" "{{lm_prefix}}/var/www-data.bak" 2>/dev/null #in case move wasn't entirely succesful (due to permission denied error on removing)
    ...
    mv "{{lm_prefix}}/www-data" "{{www_data_path}}" 2>/dev/null || mv "{{lm_prefix}}/var/www-data" "{{lm_prefix}}/var/www-data.bak" 2>/dev/null #in case move wasn't entirely succesful (due to permission denied error on removing)
    ln -s "{{www_data_path}}" "{{lm_prefix}}/var/www-data"
  else
