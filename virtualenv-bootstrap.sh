...
if [ "$1" != "noadmin" ]; then
    ...
    INSTALL=""
    if [ "$OS" == "arch" ]; then
        INSTALL="sudo pacman -Syu --needed --noconfirm base-devel pkg-config git autoconf-archive icu xml2 libxslt zlib libtar boost boost-libs python2 python python-pip python-virtualenv wget gnutls curl libexttextcat aspell"
    elif [ "$OS" == "debian" ]; then
...
pip install -U numpy
pip install -U ipython scipy matplotlib gensim scikit-learn lxml django pycrypto pandas textblob nltk
