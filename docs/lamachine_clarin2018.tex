...
environment for a developer or a good deployment method for production servers in for example CLARIN centres. 
 ...
 This paper provides a detailed description of Lamachine, its purpose and functionality and the technical details. We discuss the advantages and limitations of Lamachine to alternative software distributions in Section \ref{sec:comparison}. We demonstrate how LaMachine can create a fully functioning and standalone research environment for text mining  and NLP for Dutch texts in two use cases in Section \ref{sec:case}.
...
Central\footnote{\url{https://search.maven.org}} for Java.  LaMachine relies on those to pull and install software from
and never forks, archives, or modifies the software in any way. In doing so, we compel participating software projects
...
The software included in LaMachine has to live up (iris:ik zoek synoniem hier voor live up) to the following prerequisites:
\begin{enumerate}
...
technology-specific channels. We will elaborate on this later.
    ...
    \item {\bf maarten help even met formuleren hier, ik bedoel dat zoiets als Pattern dus helaas niet mee kan doen omdat het niet meer onderhouden wordt }only maintained/uptodate date software that can compile using the current version of compilers and dependencies/libraries
\end{enumerate}
...
In this section we present the technical design choices that were made.
...
The ICT-Research Platform Netherlands and NWO organize a yearly one-week workshop `ICT with Industry' \footnote{https://ict-research.nl/ict-with-industry/ictwi2019/} to stimulate collaboration between industry and academia. The industrical partner provides a problem and a team of researchers from different backgrounds and universities collaborate to come up with solutions. We partipated in the 2019 edition on the case study by the Dutch Royal Library who 
...
Lamachine offered a convenient platform for a range of different explorations and experiments in the area of NLP and text mining. 
However, for some situations Lamachine was not a good fit for the tasks at hand: For team members who did not have experience with a non-Windows environment, Lamachine was not a suitable or useful tool. And also for members who wanted to use a graphical user interface for their text editors as we did not manage to get the x-forwarding working (note that this also depends on the overall VM). After a few attempts the team gave up on resolving this issue die to time pressure but it also demonstrated that finetuning the configuration of Lamachine on certain aspects is beyond the reach of a data scientist without system administration skills.
 %iris it moet nog wat beter geformuleerd worden maar je snaprt wat ik bedoel
