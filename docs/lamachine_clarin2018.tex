...
We already addressed the need for different interfaces for different audiences in Section~\ref{intro}. The challenge we
...
a sufficient foundation for us to build upon. We decided this was not the case for a number of reasons: Anaconda
...
does so using its own package manager and package repositories. This would require repackaging certain software for the
anaconda ecosystem, especially handling of non-Python software was not trivial.  Whereas LaMachine consists of a series
of Ansible scripts for the installation and configuration of software, Anaconda has its own package manager (conda)
that draw from its own package repositories (conda-forge).
