...
The two native options, local and global installation, support a variety of major GNU/Linux distributions: Debian,
Ubuntu, Arch Linux, CentOS, Fedora and, to a more limited degree, we also support macOS (the local installation flavour), powered in part by the homebrew package
...
software supports it. Linux Distributions that are derivates of these \emph{may} also work, such as RedHat Enterprise Linux (CentOS), Linux Mint (Ubuntu),
...
manifest}. This set is never static but can be customized by the user. The bootstrap procedure, shown in
