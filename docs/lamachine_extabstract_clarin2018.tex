...
the CodeMeta standard\footnote{\url{https://codemeta.github.io/}} where possible, or encouraging software developers to provide
...
will merely mention some CLARIN/CLARIAH-funded tools: ucto (a tokeniser), Frog (an NLP suite for Dutch), FoLiA (Format
...
Amsterdam. Moreover, LaMachine incorporates a large number of renowned tools by external parties, offering most notably
a mature Python environment\footnote{scipy, numpy, scikit-learn, matplotlib, nltk, spacy, pytorch, keras, gensim,
etc...}, but also offering space for e.g. R and Java.

\section{Future}

The very recent release of version 2, which constituted a full rewrite, has opened up LaMachine to outside contribution.
Contributor documentation has been written, and at this stage, we greatly welcome external participants to join in. The
main objectives for the future are to providing greater \emph{interoperability} between the included tools through
better \emph{higher-level interfaces} for the researcher. We see this as a bottom-up process and have now established a
firm foundation to build upon. LaMachine development presently takes place in the scope of the CLARIAH WP3 Virtual
Research Environment project, which has much higher ambitions in accommodating the researcher and connectivity of
data and services. A major part of our future focus will be on interoperability with the higher-level tooling developed there, or in the
wider CLARIN/CLARIAH commmunity.
