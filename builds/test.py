...
def test(build, args):
    ...
    if build['flavour'] == 'local':
        ...
        cmd = "docker build -t proycon/lamachine:" + build['name'] + " --build-arg NAME=" + build['name'] + " --build-arg VERSION=" + build['version'] + " --build-arg BRANCH=master . 2>> " + cwd + "/logs/" + buildid(build).replace(':','-') + '.log >&2' #TODO: change branch to  master for release!
        r = os.system(cmd)
        os.chdir(cwd)
    else:
